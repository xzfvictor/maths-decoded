...
or fully authored topics. **More videos to come** — Year 8 (49 lessons)
and Year 10A (57 lessons) already have narrated MP4s in
`public/video/lessons/`; other modules will follow.
...
lessons. Other modules (Year 7, 9, 10, VCE) will follow.
...
- `manimcommunity/manim:latest` Docker image renders 720p30 MP4s.
...
4. **Every text label, equation, number, and example in the visual
   ...
   source of truth; if the scene shows `f(x) = 2x+1` with `f(2) = 5`
   but the audio says `f(x) = 3x+1` with `f(2) = 7`, the scene is wrong
