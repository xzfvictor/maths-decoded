...
or fully authored topics. **183 narrated video lessons** are now committed
in `public/video/lessons/`: 49 for Year 8, 57 for Year 10A, and 77 for Year
10. More videos to come for Year 7, Year 9, and VCE.
...
## Lesson videos (more to come)

Many lessons also include a **short narrated MP4** that animates the
lesson content while the narration plays. Videos live at
`public/video/lessons/{topic}/{lesson}.mp4` and are produced by
`scripts/videos/<lesson>.py` (one Manim `Scene` per lesson). The video
sources the audio from `public/audio/lessons/{topic}/{lesson}.mp3`,
which is pre-generated by `generate-audio` above.

**Coverage as of this writing:** 49 Year 8 lessons + 57 Year 10A
lessons + 77 Year 10 lessons (183 total). Other modules (Year 7, 9,
VCE) will follow.

**The pipeline lives in `scripts/videos/`:**

- `scripts/videos/_common.py` — shared helpers: `BAND_TITLE = UP*3.0`,
  `BAND_BOTTOM = DOWN*2.8`, `beat_group(*mobjects)` for clean per-beat
  fadeouts, `make_term_card()`, `make_equation_card()`,
  `animate_intro()`, `animate_final_definition()`, and the colour palette
  (BLUE_TERM, ORANGE_TERM, GREEN_OK, etc.).
- `scripts/videos/<lesson>.py` — one Manim `Scene` per lesson, each
  following a 5-beat structure (title → concrete example →
  generalisation → contrast/reject → final takeaway). The audio is the
  source of truth for every number, formula, and example.
- `scripts/videos/_render.sh` — Docker renderer/muxer; it measures the
  source MP3 and trims the final MP4 to that exact duration.
- `scripts/videos/verify_year10.sh` — verifies all 77 Year 10 outputs,
  duration alignment, and extracts 1-fps frame samples for inspection.
- `manimcommunity/manim:latest` Docker image renders the scenes.
- `ffmpeg` muxes the audio narration on top via
  `-c:v copy -c:a aac -b:a 128k -shortest`.

**MANDATORY verification before any video is declared done.** Last-frame
checks hide most mid-video issues. The pipeline always:

1. Extracts frames at 1 fps from the muxed MP4
   (`public/video/lessons/{topic}/{lesson}.mp4`) via ffmpeg.
2. Inspects at least **10–15 of them** spread across the timeline
   (0:05, 0:10, 0:15, 0:20, 0:25, 0:30, 0:40, 0:50, 1:00, 1:10,
   plus any transition frame where one beat FadeOuts and the next
   FadeIns).
3. Looks for: title covered by beat content, subtitle covered by beat
   content, two text labels overlapping, an axis/shape extending past
   the safe area (`y ∈ [-1.5, 1.8]`), multiple beats visible at once.
4. **Every text label, equation, number, and example in the visual
   content must match the audio transcript exactly.** The audio is the
   source of truth; if the scene shows `f(x) = 2x+1` with `f(2) = 5` but
   the audio says `f(x) = 3x+1` with `f(2) = 7`, the scene is wrong and
   must be rewritten.

A reusable playbook of safe-area rules, beat_group usage, scaling
limits, and the intermediate-frame verification workflow is in the
`~/.claude/skills/math-videos/SKILL.md` skill — read it before
generating any new video. **Always read the audio transcript first**
before writing the scene, and rewrite any scene whose visual content
doesn't match what the narrator says.

## Next up

Year 10 is now the latest fully authored module with narrated videos. Likely
next directions are adding videos for Year 7, Year 9, and the VCE units, or
enriching existing topics with more curated exercises. Any new topic follows
the same registration pattern (`Topic` shape, registered in the `TOPICS` array)
— `check:coverage` will fail loudly if a `dotPoints` id is unknown or unmapped.

The Year 10 video rollout is complete only when `scripts/videos/verify_year10.sh`
reports 77/77 passing after the mandatory intermediate-frame review.