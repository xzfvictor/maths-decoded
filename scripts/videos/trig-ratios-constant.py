...
Target duration: ~80.17 s (matches the audio narration length).
"""
...
class TrigRatiosConstantScene(Scene):
    def construct(self) -> None:
        ...
        # ──────────────────────────────────────────────────────────────────
        # Beat 2 — Two similar right triangles share sin theta (~20 s)
        # ──────────────────────────────────────────────────────────────────
        # Small 3-4-5 (opposite = 3, hypotenuse = 5) and a 6-8-10 (k = 2).
        small = Polygon(
            [-5.5, -1.0, 0], [-5.5, 2.0, 0], [-2.5, -1.0, 0],
            color=BLUE_TERM, stroke_width=4,
        )
        big = Polygon(
            [1.0, -2.0, 0], [1.0, 4.0, 0], [7.0, -2.0, 0],
        ...
        self.play(FadeIn(small_anno_bg, run_time=0.3), FadeIn(small_anno, run_time=0.9))
        self.play(FadeIn(big_anno_bg, run_time=0.3), FadeIn(big_anno, run_time=0.9))
        self.wait(2.5)

        # Show the two ratios side by side.
        r1 = MathTex(r"\sin\theta = \dfrac{3}{5} = 0.6", color=BLUE_TERM).scale(0.95)
        ...
        r2 = MathTex(r"\sin\theta = \dfrac{6}{10} = 0.6", color=TEAL_TERM).scale(0.95)
        ...
        self.play(FadeIn(r1_bg, run_time=0.4), Write(r1, run_time=1.6))
        self.wait(0.5)
        self.play(FadeIn(r2_bg, run_time=0.4), Write(r2, run_time=1.6))
        self.wait(2.5)
        ...
        # ──────────────────────────────────────────────────────────────────
        # Beat 4 — Same for cos and tan — quick win (~5 s)
        # ──────────────────────────────────────────────────────────────────
        same = MathTex(
        ...
        self.play(FadeIn(same_bg, run_time=0.4), Write(same, run_time=1.6))
        self.wait(1.0)

        note = Text(
            "All three ratios depend only on the angle.",
        ...
        beat4_group = VGroup(same, same_bg, note, note_bg)
        self.play(FadeOut(beat4_group, run_time=1.2))

        # ──────────────────────────────────────────────────────────────────
        # Beat 5 — Final boxed takeaway (held; total ≈ 80 s)
        # ──────────────────────────────────────────────────────────────────
        animate_final_definition(
            self,
