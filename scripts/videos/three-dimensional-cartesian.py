...
class ThreeDimensionalCartesianScene(Scene):
    def construct(self) -> None:
        ...
        # Keep y-axis tip within the safe area (y ≤ 1.5); the y-axis must
        # never extend up into the subtitle/title band.
        y_axis = Arrow(start=[-2.5, -1.4, 0], end=[-2.5, 1.5, 0],
                       color=BLUE_TERM, buff=0, stroke_width=4)
        x_lbl = MathTex("x", color=BLUE_TERM).scale(1.0).next_to(x_axis, RIGHT, buff=0.2)
        y_lbl = MathTex("y", color=BLUE_TERM).scale(1.0).next_to(y_axis, UP, buff=0.2)
        # Place the y-label just above the (now shorter) y-axis tip — above
        # y = 1.5 but well below the subtitle at y ≈ 2.4.
        y_lbl.move_to([y_axis.get_start()[0], 1.7, 0])
        ...
        # Now add the z-axis going up. Shrink so it stays within the safe
        # area (y ∈ [-1.4, 1.8]); the original y = 4.5 tip collided with the
        # title at y = 3.0+. Also offset the start to x = -1.5 (instead of
        # -2.5) so the new z axis doesn't sit on top of the y axis during
        # the Create animation (the y axis is still on screen).
        z_axis = Arrow(start=[-1.5, 0.5, 0], end=[-1.5, 1.8, 0],
                       color=ORANGE_TERM, buff=0, stroke_width=4)
        z_lbl = MathTex("z", color=ORANGE_TERM).scale(1.0).next_to(z_axis, UP, buff=0.15)
        z_lbl.move_to([-1.5, 1.95, 0])
        ...
        # ax_z tip was at y = 2.5 — collides with the subtitle. Collapse
        # to y = 1.8 (top of beat content band) and reposition label.
        ax_z = Arrow(start=[-5.0, -1.5, 0], end=[-5.0, 1.8, 0],
                     color=ORANGE_TERM, buff=0, stroke_width=4)
        lbl_x = MathTex("x", color=BLUE_TERM).scale(1.0).next_to(ax_x, RIGHT, buff=0.2)
        lbl_y = MathTex("y", color=TEAL_TERM).scale(1.0).next_to(ax_y, UR, buff=0.2)
        lbl_z = MathTex("z", color=ORANGE_TERM).scale(1.0).next_to(ax_z, UP, buff=0.15)
        # Override next_to: place the label to the LEFT of the tip so it
        # never climbs into the subtitle band.
        lbl_z.move_to([-5.5, 1.5, 0])
        ...
        z_dir = np.array([0.0, 3.3, 0.0]) / 1.0       # z-axis runs from -5,-1.5 → -5,1.8 (shrunk)
        # Scale z slightly smaller than x/y so the (1,2,3) point lands
        # within the safe area (y ≤ 1.8). 0.6 keeps pos_y ≈ 1.43.
        sx = 0.8; sy = 0.8; sz = 0.6
        ...
        # Position the (1,2,3) label to the LEFT of the point (so it never
        # climbs into the subtitle band at y ≈ 2.3). The point sits at
        # roughly (-0.7, 2.0); place the label at x ≈ -2.0, y ≈ 1.4.
        p3_lbl = MathTex("(1,\,2,\,3)", color=GREEN_OK).scale(0.9).move_to([-2.0, 1.4, 0])
