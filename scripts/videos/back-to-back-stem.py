...
Target duration: ≈ 32 s (matches audio + 20 s hold).
"""
...
class BackToBackStemScene(Scene):
    def construct(self) -> None:
        ...
        # ──────────────────────────────────────────────────────────────────
        # Beat 2 — Build the plot: stems 5-9 (~10 s)
        # ──────────────────────────────────────────────────────────────────
        # Class A scores:  52, 65, 70, 74, 88  (left side, reversed)
        # Class B scores:  60, 64, 71, 79, 90  (right side)
        # Stems: 5, 6, 7, 8, 9.
        stems = [5, 6, 7, 8, 9]
        class_a = {5: [2], 6: [5], 7: [0, 4], 8: [8], 9: []}
        class_b = {5: [],   6: [0, 4], 7: [1],   8: [],   9: [0]}
        ...
        # Two header labels.
        head_a = Text("Class A", font_size=20, color=BLUE_TERM).move_to([-2.4, y_top + 0.5, 0])
        head_a_bg = BackgroundRectangle(head_a, color=BLACK, fill_opacity=0.95, buff=0.1)
        head_a_bg.move_to(head_a.get_center())
        head_b = Text("Class B", font_size=20, color=TEAL_TERM).move_to([2.4, y_top + 0.5, 0])
        head_b_bg = BackgroundRectangle(head_b, color=BLACK, fill_opacity=0.95, buff=0.1)
        head_b_bg.move_to(head_b.get_center())

        self.play(
            FadeIn(head_a_bg, run_time=0.2), FadeIn(head_a, run_time=0.5),
            FadeIn(head_b_bg, run_time=0.2), FadeIn(head_b, run_time=0.5),
        )
        self.wait(0.5)
        ...
        for i, s in enumerate(stems):
            y = row_ys[i]
            st = MathTex(f"{s}", color=WHITE).scale(1.0).move_to([0, y, 0])
            st_bg = BackgroundRectangle(st, color=BLACK, fill_opacity=1, buff=0.16)
            ...
            at = MathTex(a_str, color=a_color).scale(0.85).move_to([-1.6, y, 0])
            at_bg = BackgroundRectangle(at, color=BLACK, fill_opacity=0.9, buff=0.1)
            ...
            bt = MathTex(b_str, color=b_color).scale(0.85).move_to([1.6, y, 0])
            bt_bg = BackgroundRectangle(bt, color=BLACK, fill_opacity=0.9, buff=0.1)
            bt_bg.move_to(bt.get_center())
            leaves_b_text.add(VGroup(bt_bg, bt))

            self.play(
                FadeIn(stem_text[i], run_time=0.4),
                FadeIn(leaves_a_text[i], run_time=0.4),
                FadeIn(leaves_b_text[i], run_time=0.4),
            )
            self.wait(0.2)
        ...
        # ──────────────────────────────────────────────────────────────────
        # Beat 3 — Reading direction (~3 s)
        # ──────────────────────────────────────────────────────────────────
        # Highlight a Class A row and a Class B row.
        pick_a = SurroundingRectangle(leaves_a_text[2], color=GREEN_OK, buff=0.15, stroke_width=3)
        pick_b = SurroundingRectangle(leaves_b_text[1], color=GREEN_OK, buff=0.15, stroke_width=3)
        self.play(Create(pick_a, run_time=0.6))
        self.wait(0.3)
        self.play(Create(pick_b, run_time=0.6))
        self.wait(0.5)

        # Reading-rule caption sits below the plot.
        rule = Text(
            "Left leaves: right-to-left (closest to stem first).",
            font_size=20, color=BLUE_TERM,
        ).move_to(BAND_CHART_CENTER + DOWN * 2.0)
        rule_bg = BackgroundRectangle(rule, color=BLACK, fill_opacity=0.95, buff=0.16)
        rule_bg.move_to(rule.get_center())
        self.play(FadeIn(rule_bg, run_time=0.3), FadeIn(rule, run_time=1.0))
        self.wait(0.8)

        # ──────────────────────────────────────────────────────────────────
        # Beat 4 — Reject: same stem is not the same value (~3 s)
        # ──────────────────────────────────────────────────────────────────
        beat3_group = VGroup(
        ...
        self.play(FadeIn(bad_bg, run_time=0.3), Write(bad, run_time=1.0))
        self.wait(0.4)
